...
export const CharacterAvatar: React.FC<CharacterAvatarProps> = ({ mood, type, style }) => {
  ...
  return (
    <div className="relative w-40 h-40 sm:w-64 sm:h-64 mx-auto z-20">
      <style>{`
        @keyframes breathe {
      ...
      <div className={`w-full h-full relative transition-transform duration-500 ${animationClass}`}>
        ...
        {/* Ears */}
        <div className={`absolute top-4 left-4 w-12 h-12 sm:w-16 sm:h-16 ${colors.ears} rounded-full`} />
        <div className={`absolute top-4 right-4 w-12 h-12 sm:w-16 sm:h-16 ${colors.ears} rounded-full`} />
        
        {type !== 'frog' && (
          <>
             <div className={`absolute top-6 left-6 w-8 h-8 sm:w-10 sm:h-10 ${colors.inner} rounded-full opacity-60`} />
             <div className={`absolute top-6 right-6 w-8 h-8 sm:w-10 sm:h-10 ${colors.inner} rounded-full opacity-60`} />
          </>
        )}

        {/* Unicorn Horn */}
        {type === 'unicorn' && (
            <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-8 h-16 bg-yellow-200 rounded-full clip-triangle z-0" style={{ clipPath: 'polygon(50% 0%, 0% 100%, 100% 100%)' }}></div>
        )}
        ...
        <div className={`absolute inset-4 ${colors.body} rounded-full shadow-lg flex flex-col items-center justify-center z-10`}>
          ...
          {/* Eyes */}
          <div className="flex justify-between w-20 sm:w-24 mb-2">
            {/* Left Eye */}
            <div className={`w-5 h-5 sm:w-6 sm:h-6 bg-amber-950 rounded-full relative overflow-hidden transition-all duration-300 ${eyeShapeClass} ${blink ? 'scale-y-10' : ''}`}>
          ...
          {/* Snout */}
          <div className={`${colors.snout} w-24 h-16 sm:w-28 sm:h-20 rounded-[50%] flex flex-col items-center justify-center relative shadow-inner-sm`}>
            {/* Nose */}
            <div className={`w-6 h-4 sm:w-8 sm:h-6 ${type === 'unicorn' ? 'bg-pink-500' : 'bg-amber-900'} rounded-full mb-1 shadow-sm`}></div>
          ...
          {/* Cheeks */}
          <div className="absolute top-24 left-4 w-6 h-3 sm:w-8 sm:h-4 bg-pink-300 rounded-full blur-md opacity-50"></div>
          <div className="absolute top-24 right-4 w-6 h-3 sm:w-8 sm:h-4 bg-pink-300 rounded-full blur-md opacity-50"></div>
        </div>
      </div>
      
      {/* Accessories */}
      {style === 'bowtie' && (
        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1 z-20 flex flex-col items-center animate-breathe">
           <div className="flex">
             <div className="w-8 h-8 bg-red-500 rounded-l-lg clip-triangle" style={{clipPath: 'polygon(0 0, 100% 50%, 0 100%)'}}></div>
             <div className="w-4 h-8 bg-red-600 rounded-sm"></div>
             <div className="w-8 h-8 bg-red-500 rounded-r-lg clip-triangle" style={{clipPath: 'polygon(100% 0, 0 50%, 100% 100%)'}}></div>
           </div>
        </div>
      )}

      {style === 'hairbow' && (
         <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 z-20 animate-breathe">
             <div className="flex relative">
                <div className="w-8 h-8 bg-pink-500 rounded-full absolute -left-4"></div>
                <div className="w-8 h-8 bg-pink-500 rounded-full absolute -right-4"></div>
                <div className="w-4 h-4 bg-pink-400 rounded-full relative z-10"></div>
             </div>
         </div>
      )}
    </div>
